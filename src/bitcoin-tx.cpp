...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " cpuchain-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  cpuchain-tx [options] <hex-tx> [commands]  Update hex-encoded cpuchain transaction\n"
                "or:     cpuchain-tx [options] -create [commands]   Create hex-encoded cpuchain transaction\n"
                "\n";
